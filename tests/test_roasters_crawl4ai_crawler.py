...
crawler_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'scrapers', 'roasters-crawl4ai', 'crawler.py'))
...
async def test_extract_roaster_minimal(monkeypatch):
    ...
    monkeypatch.setattr('scrapers.roasters_crawl4ai.enricher.enrich_missing_fields', AsyncMock(return_value={
        "name": "Test Roaster", "website_url": "https://test.com", "slug": "test-roaster", "country": "India", "is_active": True, "is_verified": False, "domain": "test.com"
    }))
...
async def test_extract_roaster_handles_missing(monkeypatch):
    ...
    monkeypatch.setattr('scrapers.roasters-crawl4ai.enricher.enrich_missing_fields', AsyncMock(return_value={
        "name": "Test Roaster", "description": "desc", "founded_year": 2020, "address": "addr"
    }))
    crawler = RoasterCrawler()
    result = await crawler.extract_roaster("Test Roaster", "https://test.com")
