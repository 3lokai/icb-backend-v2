...
async def test_default_behavior_cache_miss(
    ...
    mock_get_cached_products,
    mock_cache_products,
    mock_extract_shopify,
    mock_extract_woocommerce,
    mock_discover_crawl4ai,
    mock_enrich,
    mock_is_coffee,
    mock_validate,
    mock_dict_to_pydantic,
):
    ...
    scraper_instance.platform_detector.detect.assert_called_once_with(
        SAMPLE_URL
    )  # detect is on the instance from fixture
...
async def test_use_enrichment_false_skips_enrichment_call(
    ...
    mock_dict_to_pydantic.assert_called_once_with(
        expected_data_for_pydantic, Coffee, preprocessor=preprocess_coffee_data
    )
    mock_cache_products.assert_called_once()
...
async def test_shopify_low_confidence_falls_back_to_crawl4ai(
    ...
    await scraper_instance.scrape_products(SAMPLE_ROASTER_ID, SAMPLE_URL, SAMPLE_ROASTER_NAME)
...
async def test_force_refresh_with_use_enrichment_false(
    ...
    expected_data_for_pydantic_force_refresh_no_enrich = {**RAW_PRODUCT_1}
    mock_dict_to_pydantic.assert_called_once_with(
        expected_data_for_pydantic_force_refresh_no_enrich, Coffee, preprocessor=preprocess_coffee_data
    )
    mock_cache_products.assert_called_once()
