...
class ScraperCache:
    ...
    def get_cached_html(self, url: str, max_age_days: int = 7, field_stability: str = None) -> Optional[str]:
        """
    ...
    def get_cached_roaster(self, name: str, url: str, max_age_days: int = 30, field_stability: str = None) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_cached_products(self, roaster_id: str, max_age_days: int = 7, field_stability: str = None) -> Optional[List[Dict[str, Any]]]:
        """
        ...
        file_age = datetime.now() - datetime.fromtimestamp(cache_file.stat().st_mtime)
        if file_age > timedelta(days=max_age_days):
            logger.debug(f"Cache for roaster products {roaster_id} is {file_age.days} days old, exceeding max age of {max_age_days} days")
            return None
    ...
    def clear_cache(self, cache_type: str = None, roaster_id: str = None, url: str = None) -> int:
...
def get_cached_html(url: str, max_age_days: int = 7) -> Optional[str]:
    """Get cached HTML content for a URL."""
    return cache.get_cached_html(url, max_age_days)

def cache_html(url: str, html_content: str) -> bool:
    """Cache HTML content for a URL."""
    return cache.cache_html(url, html_content)

def get_cached_roaster(name: str, url: str, max_age_days: int = 30) -> Optional[Dict[str, Any]]:
    """Get cached roaster data."""
    return cache.get_cached_roaster(name, url, max_age_days)

def cache_roaster(roaster: Dict[str, Any]) -> bool:
    """Cache roaster data."""
    return cache.cache_roaster(roaster)

def get_cached_products(roaster_id: str, max_age_days: int = 7) -> Optional[List[Dict[str, Any]]]:
    """Get cached products for a roaster."""
    return cache.get_cached_products(roaster_id, max_age_days)

def cache_products(roaster_id: str, products: List[Dict[str, Any]]) -> bool:
    """Cache products for a roaster."""
    return cache.cache_products(roaster_id, products)

def clear_cache(cache_type: str = None, roaster_id: str = None, url: str = None) -> int:
    """Clear cache files."""
    return cache.clear_cache(cache_type, roaster_id, url)
