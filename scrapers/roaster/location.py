...
def extract_address_from_text(text: str) -> Optional[str]:
    ...
    if len(cleaned_text) > 20 and any(
        word in cleaned_text.lower() for word in ["street", "road", "area", "nagar", "colony"]
    ):
        return cleaned_text
...
async def extract_location_from_contact_page(
    base_url: str, platform: str, force_refresh: bool = False
) -> Optional[str]:
    ...
    for suffix in contact_suffixes:
        try:
            ...
            address_blocks = soup.select(
                'address, .address, [itemprop="address"], .address-block, .location-address, div[class*="address"]'
            )
            if address_blocks:
            ...
            contact_section_selectors = selectors["contact_section"] + [
                "div[class*='contact']",
...
async def extract_location(
    roaster_data: Dict[str, Any], soup: BeautifulSoup, html_content: str, platform: str, base_url: str
) -> Dict[str, Any]:
...
def extract_location_from_page(soup: BeautifulSoup, html_content: str, platform: str) -> Optional[str]:
    ...
    if footer:
        ...
        footer_address_elements = footer.select(
            'address, .address, [itemprop="address"], .contact-info, .footer-address'
        )
