...
from common.utils import standardize_roast_level, standardize_bean_type, standardize_processing_method
...
def extract_roast_level(
    ...
    explicit_patterns = [
        (r'roast(?:ed)?\s*(?:level)?(?:\s*(?:is|:))?\s*(light|medium[\s-]*light|medium|medium[\s-]*dark|dark|city[\s-]*plus|city\+|full[\s-]*city|city|french|italian|espresso|cinnamon|filter|omni[\s-]*roast)', 0.8),
        (r'(light|medium[\s-]*light|medium|medium[\s-]*dark|dark|city[\s-]*plus|city\+|full[\s-]*city|city|french|italian|cinnamon|omni[\s-]*roast)\s+roast(?:ed)?', 0.75),
    ]
...
def extract_bean_type(
    ...
    arabica_varietals = [
        ...
        'caturra', 'catuai', 'catimor', 'pacamara', 'maragogipe', 
        'pacas', 'villa sarchi', 'mundo novo'
    ]
...
def extract_processing_method(
    ...
    explicit_patterns = [
        (r'process(?:ing)?(?:\s*(?:method|type))?(?:\s*(?:is|:))?\s*(washed|natural|honey|anaerobic|monsooned|wet[\s-]*hulled|carbonic[\s-]*maceration|double[\s-]*fermented)', 0.8),
        (r'(washed|natural|honey|anaerobic|monsooned|wet[\s-]*hulled|carbonic[\s-]*maceration|double[\s-]*fermented)(?:\s*(?:process|processing|processed))', 0.8),
    ]
...
def extract_flavor_profiles(
    ...
    known_flavors = [
        ...
        "wine", "winey", "grapes", "blackcurrant", "melon", "herbal", "roasted"
    ]
    ...
    flavor_section_patterns = [
        r'(?:FLAVOUR|FLAVOR)\s+NOTES:\s*(.*?)(?:\.|$)',
        r'Taste\s+Notes\s*[-:]\s*(.*?)(?:\.|$)'
    ]
...
def detect_is_single_origin(
    ...
    origin_indicators = [
        'estate', 'farm', 'ethiopia', 'colombian', 'kenya', 'sumatra', 
        'guatemala', 'brazil', 'costa rica', 'honduras', 'rwanda', 'burundi',
        'el salvador', 'nicaragua', 'panama', 'indonesia', 'india', 'vietnam',
        'mexico', 'peru', 'jamaica', 'hawaii', 'kona'
    ]
...
def detect_is_seasonal(
    ...
    seasonal_patterns = [
        r'\bseasonal\b', r'\blimited\s+(?:time|edition|release|availability)\b',
        r'\bavailable\s+(?:only|just)\s+for\b', r'\bspecial\s+harvest\b',
        r'\bshort\s+time\b', r'\btemporal\b', r'\bwhile\s+supplies\s+last\b'
    ]
