...
def process_variants(coffee: Dict[str, Any], product: Dict[str, Any], confidence_tracking: bool = True) -> Dict[str, Any]:
    """
...
def process_woocommerce_variants(coffee: Dict[str, Any], variations: List[Dict[str, Any]], confidence_tracking: bool = True) -> Dict[str, Any]:
    """
...
def extract_weight_price_map_from_html(html: str) -> List[Tuple[int, float, float]]:
    ...
    for block in variation_blocks:
        ...
        for value, label in options:
            ...
            price_match = re.search(fr'data-value="{re.escape(value)}"[^>]*>.*?([0-9,.]+)', block, re.DOTALL)
            if price_match:
                try:
    ...
    radio_patterns = re.findall(r'<input[^>]*type="radio"[^>]*value="([^"]*)"[^>]*>[^<]*([^<]+).*?([0-9,.]+)', html, re.DOTALL)
...
def validate_price_logic(coffee: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for i in range(len(sorted_prices) - 1):
        ...
        if next_ppm > current_ppm * 1.1:  # Allow a small margin for rounding errors
            ...
            logger.warning(f"Price inconsistency: {weights[next_key]}g pack has higher price per gram than {weights[current_key]}g pack")
