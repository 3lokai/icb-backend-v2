...
async def process_csv_batch(csv_path: str, output_path: str = None, limit: int = None, concurrency: int = 5, rate_limit: int = 10, rate_period: float = 60.0):
    ...
    # Process batch
    print(f"Processing {len(roaster_list)} roasters (concurrency={concurrency}, rate_limit={rate_limit}/{rate_period}s)...")
    results = await batch_process_roasters(roaster_list, concurrency=concurrency, rate_limit=rate_limit, rate_period=rate_period, export_path=output_path)
...
async def main():
    ...
    parser.add_argument("-s", "--single", action="store_true", help="Process a single roaster (input should be name,url)")
    ...
    if args.single:
        ...
        if errors:
            ...
            for error in errors[:5]:  # Show first 5 errors
                print(f"  {error.get('name', 'Unknown')} ({error.get('url', 'No URL')}): {error.get('error', 'Unknown error')}")
