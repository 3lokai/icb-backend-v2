...
from common.utils import is_coffee_product, ensure_absolute_url
...
async def discover_products_via_crawl4ai(
    ...
    url_filters = FilterChain([
        ...
        URLPatternFilter(patterns=[
            ...
            "*/coffee/*", "*/coffees/*", "*/beans/*", "*coffee*.html",
            "*/single-origin/*", "*/blend/*", "*/roast/*", "*/arabica/*",
            # Product detail patterns (avoiding category pages)
            "*-coffee.html", "*-blend.html", "*-beans.html", "*-roast.html",
            # Known product naming patterns
            "*estate*", "*origin*", "*filter*-coffee*",
    ...
    keywords=[
        "coffee", "bean", "roast", "arabica", "robusta", "espresso", "product",
        "single origin", "blend", "light", "medium", "dark", "buy", "purchase",
        # Additional scoring keywords
        "detail", "view", "item", "filter", "specialty", "brewing"
    ],
    weight=0.8
)
        ...
        try:
            async for result in await crawler.arun(url=base_url, config=config):
                ...
                if is_product_page(result.url, result.html, result.markdown):
                    ...
                    product_name = ""
                    title_match = re.search(r'<h1[^>]*>([^<]+)</h1>', result.html) or re.search(r'<title>([^<|]+)', result.html)
                    if title_match:
                        product_name = title_match.group(1).strip()
                    # Extract description
                    description = ""
                    desc_match = re.search(r'<meta\s+name=["|\']description["|\'][^>]*content=["|\']([^"\']+)["|\']', result.html)
                    if desc_match:
...
def is_product_page(url: str, html: str, markdown: str) -> bool:
    ...
    cart_patterns = [
        r'add[_ -]to[_ -]cart', r'add[_ -]to[_ -]bag', 
        r'buy[_ -]now', r'purchase', r'checkout'
    ]
    ...
    coffee_patterns = [
        r'roast level', r'origin', r'bean type', r'single origin',
        r'tasting notes', r'flavor notes', r'processing method',
        r'arabica', r'robusta', r'altitude', r'brewing'
    ]
    coffee_matches = sum(1 for pattern in coffee_patterns 
    ...
    url_patterns = [
        r'/product/', r'/products/', r'/coffee/', r'/coffees/', r'/beans/',
        r'/p/[^/]+$', r'\.html$', r'/item/', r'/buy/'
    ]
    ...
    if is_product:
        ...
        if product_name:
            return validate_product_at_discovery(
                name=product_name,
                description=description,
                url=url
            )
