...
def validate_roast_level(value: str) -> ValidationResult:
    ...
    if not value:
        return ValidationResult("roast_level", False, ValidationLevel.WARNING, "Roast level is empty or not specified")
    ...
    return ValidationResult("roast_level", False, ValidationLevel.WARNING, f"Invalid roast level: '{value}'")
...
def validate_bean_type(value: str) -> ValidationResult:
    ...
    Returns:
        ValidationResult with validation status
    """
    valid_bean_types = ["arabica", "robusta", "liberica", "blend", "mixed-arabica", "arabica-robusta", "unknown"]

    if not value:
        return ValidationResult("bean_type", False, ValidationLevel.WARNING, "Bean type is empty or not specified")
    ...
    if value in bean_mapping:
        ...
        return ValidationResult(
            "bean_type", False, ValidationLevel.INFO, f"Corrected bean type from '{value}' to '{corrected}'", corrected
        )
    ...
    if "arabica" in value:
        return ValidationResult(
            "bean_type", False, ValidationLevel.INFO, f"Corrected bean type from '{value}' to 'arabica'", "arabica"
        )

    if "robusta" in value:
        return ValidationResult(
            "bean_type", False, ValidationLevel.INFO, f"Corrected bean type from '{value}' to 'robusta'", "robusta"
        )

    if "liberica" in value:
        return ValidationResult(
            "bean_type", False, ValidationLevel.INFO, f"Corrected bean type from '{value}' to 'liberica'", "liberica"
        )

    if "blend" in value or "mix" in value:
        return ValidationResult(
            "bean_type", False, ValidationLevel.INFO, f"Corrected bean type from '{value}' to 'blend'", "blend"
        )

    return ValidationResult("bean_type", False, ValidationLevel.WARNING, f"Invalid bean type: '{value}'")
...
def validate_processing_method(value: str) -> ValidationResult:
    ...
    if not value:
        return ValidationResult(
            "processing_method", False, ValidationLevel.WARNING, "Processing method is empty or not specified"
        )
    ...
    return ValidationResult(
        "processing_method", False, ValidationLevel.WARNING, f"Invalid processing method: '{value}'"
    )


def validate_price(price: float, size_grams: Optional[int] = None) -> ValidationResult:
    """
    ...
    if price is None:
        return ValidationResult(field, False, ValidationLevel.WARNING, "Price is not specified")

    try:
        price_float = float(price)
    except (ValueError, TypeError):
        return ValidationResult(field, False, ValidationLevel.ERROR, f"Price '{price}' is not a valid number")

    # Check if price is negative
    if price_float < 0:
        return ValidationResult(field, False, ValidationLevel.ERROR, f"Price '{price_float}' is negative")

    # Check if price is zero
    if price_float == 0:
        return ValidationResult(field, False, ValidationLevel.WARNING, "Price is zero")
    ...
    if size_grams:
        ...
        if price_per_gram < 0.01:  # Extremely low price per gram
            return ValidationResult(
                field, False, ValidationLevel.WARNING, f"Price '{price_float}' seems unusually low for {size_grams}g"
            )

        if price_per_gram > 0.25:  # Extremely high price per gram
            return ValidationResult(
                field, False, ValidationLevel.WARNING, f"Price '{price_float}' seems unusually high for {size_grams}g"
            )
    else:
        # If no size provided, do basic sanity checks
        if price_float < 5:  # Extremely low absolute price
            return ValidationResult(field, False, ValidationLevel.WARNING, f"Price '{price_float}' seems unusually low")

        if price_float > 10000:  # Extremely high absolute price
            return ValidationResult(
                field, False, ValidationLevel.WARNING, f"Price '{price_float}' seems unusually high"
            )
...
def validate_multi_size_prices(coffee: Dict[str, Any]) -> List[ValidationResult]:
    ...
    sizes = {"price_100g": 100, "price_250g": 250, "price_500g": 500, "price_1kg": 1000}
    ...
    if (
        coffee.get("is_multipack")
        or "pack of" in coffee.get("name", "").lower()
        or "multipack" in coffee.get("name", "").lower()
    ):
...
def validate_flavor_profiles(profiles: List[str]) -> ValidationResult:
    ...
    """
    if not profiles:
        return ValidationResult("flavor_profiles", False, ValidationLevel.WARNING, "Flavor profiles are not specified")
...
def validate_url(url: str, field_name: str = "url") -> ValidationResult:
    ...
    """
    if not url:
        return ValidationResult(field_name, False, ValidationLevel.WARNING, f"{field_name} is not specified")
    ...
    return ValidationResult(field_name, False, ValidationLevel.WARNING, f"Invalid {field_name}: '{url}'")
...
def validate_coffee_product(coffee: Dict[str, Any]) -> List[ValidationResult]:
    ...
    if "name" not in coffee or not coffee["name"]:
        results.append(ValidationResult("name", False, ValidationLevel.ERROR, "Product name is required"))

    if "direct_buy_url" not in coffee or not coffee["direct_buy_url"]:
        results.append(ValidationResult("direct_buy_url", False, ValidationLevel.ERROR, "Direct buy URL is required"))
    else:
    ...
    # Price validations
    price_fields = {"price_100g": 100, "price_250g": 250, "price_500g": 500, "price_1kg": 1000}
